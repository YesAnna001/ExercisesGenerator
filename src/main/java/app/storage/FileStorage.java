...
public class FileStorage {
	...
	private void ensureDirs() {
		...
		if (!exercisesCsv.exists()) {
			writeAllLines(exercisesCsv, Collections.singletonList("id,createdAtMs,count,creator,published,publishedAtMs,type"));
		}
		if (!scoresCsv.exists()) {
	...
	public List<QuestionBank> loadAllBanks() {
		...
		for (String line : lines) {
			...
			String type = a[6];
			list.add(new QuestionBank(id, created, count, creator, published, publishedAt,type));
		}
	...
	public QuestionBank loadBank(String bankId) {
		...
		for (String line : lines) {
			...
			if (bankId.equals(a[0])) {
				...
				long publishedAt = parseLong(a[5]);
				String type = a[6];
				return new QuestionBank(id, created, count, creator, published, publishedAt, type);
			}
		}
		return null;
	}
